...
const getActiveComposerToken = (
  ...
  if (prefix === "@" && /^@[a-zA-Z0-9._-]*$/.test(token)) {
    return {
      ...
      query: text.slice(tokenStart + 1, caretEnd)
    };
  }
...
export const MessagingModule = ({
  ...
  const mentionsByHandle = useMemo(() => {
    ...
    for (const member of teamMembers) {
      ...
      if (!map.has(baseHandle)) {
        map.set(baseHandle, member);
      }
    }
    return map;
  }, [teamMembers]);
  ...
  const mentionOptions = useMemo(() => {
    ...
    return teamMembers
      .filter((member) => {
        ...
        return normalizeHandle(member.name).includes(normalized);
      })
      .slice(0, 6);
  ...
  useEffect(() => {
    ...
    if (dismissedTokenSignature && dismissedTokenSignature !== tokenSignature) {
      setDismissedTokenSignature(null);
    }
  ...
  const insertAtCursor = (text: string) => {
    ...
    if (!textarea) {
      setComposerValue((current) => `${current}${text}`);
  ...
  const replaceActiveToken = (replacement: string) => {
    ...
    const nextValue =
      composerValue.slice(0, activeToken.start) + replacement + composerValue.slice(activeToken.end);
  ...
  const handleInsertMention = (member: TeamMember) => {
    ...
    replaceActiveToken(`@${handle} `);
  };

  const handleInsertEmojiAutocomplete = (emoji: EmojiOption) => {
    replaceActiveToken(`${emoji.native} `);
  };
  ...
  const handleSend = () => {
    ...
    const mentionIds = [...content.matchAll(/@([a-zA-Z0-9._-]+)/g)]
      .map((match) => mentionsByHandle.get(normalizeHandle(match[1] ?? ""))?.id ?? null)
  ...
  const handleComposerKeyDown = (event: KeyboardEvent<HTMLTextAreaElement>) => {
    if (showMentionDropdown) {
      if (event.key === "ArrowDown") {
        ...
        setDropdownIndex((current) => (current + 1) % mentionOptions.length);
        return;
      }
      if (event.key === "ArrowUp") {
        event.preventDefault();
        setDropdownIndex((current) => (current - 1 + mentionOptions.length) % mentionOptions.length);
        return;
      }
      if (event.key === "Enter" || event.key === "Tab") {
        event.preventDefault();
        const member = mentionOptions[dropdownIndex] ?? mentionOptions[0];
        if (member) {
      ...
      if (event.key === "Escape") {
        event.preventDefault();
        if (tokenSignature) {
          setDismissedTokenSignature(tokenSignature);
        }
        return;
      }
    }
    ...
    if (showEmojiAutocomplete) {
      if (event.key === "ArrowDown") {
        event.preventDefault();
        setDropdownIndex((current) => (current + 1) % emojiAutocompleteOptions.length);
        return;
      }
      if (event.key === "ArrowUp") {
        event.preventDefault();
        setDropdownIndex((current) => (current - 1 + emojiAutocompleteOptions.length) % emojiAutocompleteOptions.length);
        return;
      }
      if (event.key === "Enter" || event.key === "Tab") {
        event.preventDefault();
        const emoji = emojiAutocompleteOptions[dropdownIndex] ?? emojiAutocompleteOptions[0];
        if (emoji) {
          handleInsertEmojiAutocomplete(emoji);
        }
        return;
      }
      if (event.key === "Escape") {
                    ...
                    <div className="relative h-11 w-11 shrink-0 rounded-full bg-[#128c7e] text-center text-sm font-bold leading-[2.75rem] text-white">
                      ...
                      <span
                        className={`absolute bottom-0 right-0 h-3 w-3 rounded-full border-2 border-white ${
                          conversation.isOnline ? "bg-[#25d366]" : "bg-slate-300"
                        }`}
                      />
                    </div>
                    ...
                    <article
                      className={`max-w-[70%] rounded-2xl px-3 py-2 shadow-sm ${
                        isMine
                          ...
                          : "rounded-bl-md border border-[#e4e9f0] bg-white"
                      }`}
                    >
                      {!isMine ? (
                        <p className="mb-1 text-[11px] font-semibold text-[#128c7e]">
                          {message.senderName}
                        </p>
                      ) : null}
                      ...
                      {message.kind === "FILE" && (message.attachments?.length ?? 0) > 0 ? (
                        <div className="mt-2 space-y-2">
                          {message.attachments?.map((attachment) => (
                            <div
                              key={attachment.id}
                              className="rounded-xl border border-[#e4e9f0] bg-white/80 p-2"
                            >
          ...
          <footer className="border-t border-[#e4e9f0] bg-white px-3 py-3">
            <div className="relative">
              <div className="flex items-end gap-2">
                ...
                <label className="rounded-full p-2 text-[#667781] hover:bg-[#f0f2f5]">
                  📎
              ...
              {showMentionDropdown ? (
                <div className="absolute bottom-[105%] left-0 right-0 rounded-xl border border-[#e4e9f0] bg-white p-2 shadow-lg sm:left-12 sm:right-14">
                  <p className="mb-1 text-[11px] font-semibold uppercase tracking-wide text-[#667781]">
                    Mencionar usuario
                  </p>
                      ...
                      <li key={member.id}>
                        <button
                          ...
                          className={`w-full rounded-lg px-2 py-1 text-left text-sm ${
                            index === dropdownIndex ? "bg-[#e9f8ef]" : "hover:bg-[#f0f2f5]"
                          }`}
                        >
            ...
            <div className="space-y-2">
              {teamMembers.map((member) => (
                <article key={member.id} className="rounded-xl border border-[#e4e9f0] bg-[#f9fbfd] p-2.5">
                  ...
                  {member.status ? (
                    <span
                      className={`mt-2 inline-flex rounded-full px-2 py-0.5 text-[11px] font-semibold ${statusBadge(member.status)}`}
                    >
