...
const NotificationToast = ({
  ...
  const borderColor = isUrgent ? "border-urgent/30" : "border-accent/30";
  const accentBar = isUrgent ? "bg-urgent-muted0" : "bg-accent";
    ...
    >
      ...
      <div className="flex items-start gap-3 py-3 pl-4 pr-3">
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold text-ink leading-snug">{toast.title}</p>
          {toast.body ? (
            <p className="mt-0.5 text-[11px] leading-snug text-mid">
        ...
        <button
          ...
          className="mt-0.5 shrink-0 rounded-lg p-1 text-faint transition-colors hover:bg-line hover:text-mid"
