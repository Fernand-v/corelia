...
const optimizeImageBeforeUpload = async (file: File): Promise<File> => {
  ...
  try {
    const image = await new Promise<HTMLImageElement>((resolve, reject) => {
      ...
      instance.onerror = () => reject(new Error("No se pudo leer la imagen seleccionada"));
    ...
    const scale = Math.min(1, MAX_ANNOUNCEMENT_IMAGE_DIMENSION / Math.max(sourceWidth, sourceHeight));
...
export const AdminAnnouncementComposer = () => {
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      const inferredTitleBlock = blocks.find(
        (
          block
        ): block is Extract<EditorBlock, { type: "TITLE" | "SUBTITLE" | "TEXT" }> =>
          (block.type === "TITLE" || block.type === "SUBTITLE" || block.type === "TEXT") &&
          block.text.trim().length > 0
      );
  ...
  const uploadBlockAsset = async (
    ...
    try {
      const uploadFile = blockType === "IMAGE" ? await optimizeImageBeforeUpload(file) : file;
      const formData = new FormData();
      formData.append("file", uploadFile, uploadFile.name);
      const uploaded = await apiRequest<AnnouncementAssetUploadResponse>("/announcements/assets/upload", {
        method: "POST",
        body: formData
      });
      ...
      <div className="grid gap-4 lg:grid-cols-2">
          ...
          <div className="space-y-2 rounded-xl border border-slate-200 p-3">
            ...
            <div className="flex flex-wrap gap-2">
              <Button
                ...
                onClick={() => setBlocks((current) => [...current, createBlock("TITLE")])}
              >
                + Título
              </Button>
              <Button
                ...
                variant="secondary"
                className="h-8 px-3 text-xs"
                onClick={() => setBlocks((current) => [...current, createBlock("SUBTITLE")])}
              >
                + Subtítulo
              </Button>
              <Button
                ...
                variant="secondary"
                className="h-8 px-3 text-xs"
                onClick={() => setBlocks((current) => [...current, createBlock("TEXT")])}
              >
                + Texto
              </Button>
              <Button
                ...
                variant="secondary"
                className="h-8 px-3 text-xs"
                onClick={() => setBlocks((current) => [...current, createBlock("IMAGE")])}
              >
                + Imagen
              </Button>
              <Button
                ...
                variant="secondary"
                className="h-8 px-3 text-xs"
                onClick={() => setBlocks((current) => [...current, createBlock("FILE")])}
              >
                + Archivo
              </Button>
              <Button
                ...
                variant="secondary"
                className="h-8 px-3 text-xs"
                onClick={() => setBlocks((current) => [...current, createBlock("DIVIDER")])}
              >
                + Separador
              </Button>
            </div>
                      ...
                      <label className="text-xs text-slate-600">
                        ...
                        <input
                          ...
                          onChange={(event) => {
                            ...
                            void uploadBlockAsset(block.clientId, file, "IMAGE");
                      ...
                      <label className="text-xs text-slate-600">
                        ...
                        <input
                          ...
                          onChange={(event) => {
                            ...
                            void uploadBlockAsset(block.clientId, file, "FILE");
                            event.currentTarget.value = "";
                          }}
                        />
                      </label>
        ...
        <section className="space-y-2 rounded-xl border border-slate-200 bg-slate-50 p-3">
          ...
          <article className="rounded-xl border border-slate-200 bg-white p-3">
            <h3 className="text-lg font-semibold text-slate-900">{title.trim() || "Título del anuncio"}</h3>
