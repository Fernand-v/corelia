...
export class DocumentsService {
  ...
  joinDiagramSession(input: Parameters<DocumentCollabService["joinDiagramSession"]>[0]) {
    return this.collab.joinDiagramSession(input);
  }

  heartbeatDiagramSession(input: Parameters<DocumentCollabService["heartbeatDiagramSession"]>[0]) {
    return this.collab.heartbeatDiagramSession(input);
  }

  saveDiagramSessionSnapshot(input: Parameters<DocumentCollabService["saveDiagramSessionSnapshot"]>[0]) {
    return this.collab.saveDiagramSessionSnapshot(input);
  }

  leaveDiagramSession(input: Parameters<DocumentCollabService["leaveDiagramSession"]>[0]) {
    return this.collab.leaveDiagramSession(input);
  }

  getDiagramSessionState(input: Parameters<DocumentCollabService["getDiagramSessionState"]>[0]) {
    return this.collab.getDiagramSessionState(input);
  }
  ...
  private async createVersionFromBuffer(input: {
    ...
    if (document.type === "DIAGRAMA") {
      const activeSession = await this.app.prisma.documentCollabSession.findFirst({
        where: {
          documentId: document.id,
          status: "ACTIVE"
        },
      ...
      if (activeSession) {
        await this.app.prisma.documentCollabEvent.create({
          data: {
